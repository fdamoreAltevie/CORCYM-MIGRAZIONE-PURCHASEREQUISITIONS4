		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <PurchaseRequisition>");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "ExtendedPropertiesRdaSet",
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("PurchaseRequisition") === oObject.PurchaseRequisition;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].PurchaseRequisition;
									} else {
										sCurrentId = aItemsNotInTheList[0].PurchaseRequisition;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ExtendedPropertiesRdaSet", {
										PurchaseRequisition : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("PurchaseRequisition"),
						title: oBindingContext.getProperty("PurchaseRequisition")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<ExtendedPropertiesRdaSet> (0)"}),
						success: function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("PurchaseRequisition");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all ExtendedPropertiesRdaSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<ExtendedPropertiesRdaSet> (" + iExpectedLength + ")"}),
								success : function () {
