						...
						success : function (aEntitySet) {
							...
							return this.waitFor({
								...
								check: function (oList) {
									...
									var sObjectID = oList.getBindingContext().getProperty("PurchaseRequisition");

									var iLength = aEntitySet.filter(function (oLineItem) {
										return oLineItem.PurchaseRequisition === sObjectID;
									}).length;
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							return this.waitFor({
								id : "lineItemsTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name: "text", value: "<PositionsPlural> (" + iNumberOfItems + ")"}),
								success : function () {
